...
2. 通用工具：榨季月份、评估指标、面积加权。
3. 数据与模型：读表补齐榨季字段、构造样本、训练 Ridge、还原参数。
4. 折叠与 GEE：把县级历史项折叠进截距，并生成 GEE JavaScript。
5. 本地栅格计算：读取本地月度 tif，复现 GEE 线性公式，按 512×512 窗口分块输出 GeoTIFF。
6. 平台接入：ALGO_META 参数声明 + run() 入口，供 FastAPI 平台调用。
"""
...
# =============================================================================
# ===================== 1. 数据文件与运行口径 =====================
# INPUT_FILE 是建模表 Excel。默认值相对本文件位置推导，不再依赖开发机绝对路径；
# 平台 API 运行时会被前端选中的服务器端文件路径覆盖（见文件末尾 run()）。
INPUT_FILE = (
    Path(__file__).resolve().parents[1]
    / "data"
    / "GX_County_Sugarcane_Monthly_2020_2025_with_Yield_Area.xlsx"
)
...
# ===================== 4. 表字段与原始特征声明 =====================
# 这些字段名必须和 Excel 表一致。字段名集中放在这里，是为了后续函数只引用统一变量，避免硬编码散落各处。
COUNTY_COL = "地名"
DATE_COL = "date"
TARGET_COL = "Yield"
...
# RAW_FEATURES 是真正从表中读取并可在 GEE 端逐像元计算的原始变量。
...
# HISTORY_COLS 是县级历史统计量，GEE 像元端无法逐像元计算，所以后面会折叠进截距。
HISTORY_COLS = [
...
class FoldedFormula:
    """
    保存县级模型折叠到 GEE 像元公式后的核心结果。
    raw_intercept 是 Ridge 原始线性公式截距；folded_intercept 是把县级历史项折叠后的像元公式截距。
    dynamic_params 只保留 GEE 可以逐像元计算的特征参数，fold_table 记录被折叠进截距的县级历史项贡献。
    """
    raw_intercept: float
...
def metric_dict(actual: Sequence[float], pred: Sequence[float], weights: Sequence[float] | None = None) -> Dict[str, float]:
    """
    统一计算模型评估指标，并在提供面积权重时补充省级面积加权误差。
...
def print_metrics(title: str, metrics: Dict[str, float]) -> None:
    """
    按固定顺序打印评估指标，保证训练集、目标榨季、原始单位和最终单位的格式一致。
...
def make_ridge_model():
    """
    创建 RidgeCV 建模流水线。
    SimpleImputer 负责缺失值填补，StandardScaler 负责标准化，RidgeCV 在多个 alpha 中选择正则强度。
    """
...
def fold_county_model_to_pixel_formula(
    ...
    """
    把县级 Ridge 公式拆成 GEE 像元可计算部分和不可逐像元计算部分。
    县级历史项无法在像元影像上直接计算，因此按目标区面积权重求一个代表值并折叠进截距；NDVI、温度、降水等动态项保留给 GEE 逐像元代入。
    """
...
def make_gee_pixel_formula_snippet(
    ...
    return "\n".join(
        [
            "// =====================================================================",
            "// 广西甘蔗单产实时估产：县级模型折叠后的像元级 GEE 公式",
            "// 输出 tif 不是全省常数：每个甘蔗像元按自己的 NDVI、temperature_2m、total_precipitation_sum 时序计算。",
            ...
            *param_lines,
            ...
            "",
            "// 最终结果：由像元级原始结果乘以 FINAL_RESULT_SCALE 后求均值。",
            ...
            "",
            "// 检查输出是否有像元差异：stdDev > 0 时说明不是常数 tif。",
...
def print_workflow_report(result: Dict[str, object]) -> None:
    ...
    print("原始像元公式: Raw_pixel = folded_intercept + sum(gee_raw_coef * filled_pixel_feature)")
    print("最终像元公式: Final_pixel = Raw_pixel * FINAL_RESULT_SCALE")
    ...
    print("\n" + "=" * 24 + " GEE 像元差异 tif 代码 " + "=" * 24)
    print(
        textwrap.dedent(
            """
            说明:
            1. 下方代码不是常数 tif，而是把折叠后的线性公式逐像元应用到三特征时序影像。
            2. rawYieldImage 是原始模型结果，yieldImage = rawYieldImage * FINAL_RESULT_SCALE，是最终导出的结果。
            3. pixelVariationCheck 会打印 min/max/stdDev，用来确认输出 tif 有像元差异。
            """
        ).strip()
    )
...
BACKEND_DIR = WORKSPACE.parent
INPUT_RASTER_DIR = BACKEND_DIR / "data" / "月度影像_2026"
OUTPUT_TIF = BACKEND_DIR / "outputs" / "本地持续估产_2026_06_final.tif"
RAW_OUTPUT_TIF = BACKEND_DIR / "outputs" / "本地持续估产_2026_06_raw.tif"
CANE_MASK_TIF: Path | None = None
NODATA_VALUE = -9999.0
...
def open_aligned_dataset(stack: ExitStack, path: Path, ref_grid: dict, resampling: Resampling):
    """打开一个 tif；若网格与参考影像不同，用 WarpedVRT 延迟重投影（read(window) 时只重采样当前窗口）。"""
    src = stack.enter_context(rasterio.open(path))
    if src.count != 1:
        raise ValueError(f"要求单波段 tif，但 {path} 有 {src.count} 个波段。")
    if same_grid(ref_grid, raster_profile_from_dataset(src)):
        return src
    ...
    return stack.enter_context(
        WarpedVRT(
            ...
            width=int(ref_grid["width"]),
            height=int(ref_grid["height"]),
...
def write_window_tif_outputs(
    ...
    """分块计算估产 tif，返回 final 栅格统计 {count, mean, min, max, std}。"""
    ...
    if cane_mask_path is not None and not cane_mask_path.exists():
        raise FileNotFoundError(f"甘蔗掩膜 tif 不存在: {cane_mask_path}")
    ...
    # 统计累积器（避免整幅重读）
    acc = {"count": 0, "sum": 0.0, "sumsq": 0.0, "min": np.inf, "max": -np.inf}
    ...
    with ExitStack() as stack:
        ...
        datasets: Dict[Tuple[int, str], object] = {}
        for month_idx, year, month, _ in months:
            for feature in RAW_FEATURES:
                ...
                if path.resolve() == first_path.resolve():
                    ...
                    datasets[(month_idx, feature)] = open_aligned_dataset(
                        stack, path, ref_grid, Resampling.bilinear
                    )

        mask_ds = None
        if cane_mask_path is not None:
            print(f"甘蔗掩膜 tif: {cane_mask_path}")
        ...
        with rasterio.open(raw_output_tif, "w", **out_profile) as raw_dst, \
                rasterio.open(output_tif, "w", **out_profile) as final_dst:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cane-mask-tif", default=None, help="可选甘蔗掩膜 tif，>0 像元保留。")
...
def main() -> None:
    ...
    # 真实影像单幅可达 2GB+，这里与平台 run() 一致使用分块执行器，
    # 按 512×512 窗口读特征、算公式、写结果，避免整幅读入内存。
    stats = write_window_tif_outputs(
...
# =============================================================================
# 11. 平台接入：ALGO_META 与 run()
# =============================================================================
# FastAPI 平台约定：每个算法文件暴露 ALGO_META（前端据此生成参数表单）
# 和 run(inputs, workdir)。inputs 的值是参数名到服务器端文件路径（或月份字符串）的映射，
...
ALGO_META = {
    ...
    "name": "广西甘蔗单产持续估产",
    "description": (
        "真实估产算法：读取县级建模表训练 Ridge 模型，把县级历史项折叠进截距，"
        "再对 Apr-目标月的月度 NDVI/气温/降水影像逐像元计算最终单产（t/亩）。"
        "月度影像按 {year}_{month:02d}_{feature}.tif 命名并放在同一目录（如 2026_04_NDVI.tif），"
        "算法按目标月份在该目录自动查找其余影像；甘蔗掩膜可选，用于限定计算范围。"
    ),
    "params": [
        {"name": "excel", "label": "县级建模表 Excel", "type": "xlsx", "required": True},
        {
            "name": "raster_sample",
            "label": "月度影像样例 tif（同目录其余影像自动发现）",
            "type": "tif",
            "required": True,
        },
        {"name": "cane_mask", "label": "甘蔗掩膜 tif", "type": "tif", "required": False},
        {"name": "target_month", "label": "目标月份", "type": "month", "required": True, "default": "2026-06"},
    ],
}
...
def run(inputs: dict, workdir: Path) -> dict:
    """平台约定入口：训练县级模型 → 折叠参数 → 分块逐像元计算 → 组装返回结构。"""
    workdir = Path(workdir)
    target_month = inputs["target_month"]
    excel_path = Path(inputs["excel"])
    raster_sample = Path(inputs["raster_sample"])
    input_dir = raster_sample.parent
    mask_path = Path(inputs["cane_mask"]) if inputs.get("cane_mask") else None

    # 1. 县级表训练 + 参数折叠（目标月份决定 Apr 到哪个月的特征截断）
    args = argparse.Namespace(
        ...
        allow_target_in_training=ALLOW_TARGET_IN_TRAINING,
        preview_rows=10,
        gee_sugarcane_asset=GEE_SUGARCANE_ASSET,
        gee_province_asset=GEE_PROVINCE_ASSET,
        gee_export_folder=GEE_EXPORT_FOLDER,
        gee_export_prefix=GEE_EXPORT_PREFIX,
        final_result_scale=FINAL_RESULT_SCALE,
    )
    model = run_folded_formula_workflow(args)
    folded = model["folded_full"]

    # 2. 分块逐像元栅格计算（大影像按 512×512 窗口读写，网格不一致自动重投影）
    raw_tif = workdir / "估产结果_raw.tif"
    final_tif = workdir / "估产结果_final.tif"
    stats = write_window_tif_outputs(
        target_month=target_month,
        input_dir=input_dir,
        template=RASTER_NAME_TEMPLATE,
        folded=folded,
        final_scale=model["result_scale"],
        cane_mask_path=mask_path,
        raw_output_tif=raw_tif,
        output_tif=final_tif,
        nodata=NODATA_VALUE,
    )

    # 3. 组装平台返回结构：float 指标 / csv 表格 / tif 结果
    ...
    metrics = [
        m
        for m in [
            push_metric("全省平均预测单产", model["folded_formula_full_final"], "t/亩"),
            push_metric("训练集 R²", model["train_metrics"]["R2"]),
            push_metric("训练集 MAE", model["train_metrics_final"]["MAE"], "t/亩"),
            push_metric("栅格平均单产", stats["mean"], "t/亩"),
            push_metric("栅格最小单产", stats["min"], "t/亩"),
            push_metric("栅格最大单产", stats["max"], "t/亩"),
            push_metric("有效像元数", stats["count"], "个", 0),
        ]
        if m
    ]

    # 县级预测结果表
    county_cols = ["地名", "目标榨季", "历史单产均值(t/ha)", "预测单产(t/ha)", "最终预测单产(t/亩)", "参考面积(ha)", "观测月数"]
    county_df = model["target"].rename(
        columns={
            COUNTY_COL: "地名",
            SEASON_COL: "目标榨季",
            "county_yield_hist_mean": "历史单产均值(t/ha)",
            "predicted_yield": "预测单产(t/ha)",
            "final_predicted_yield": "最终预测单产(t/亩)",
            "area_for_weight": "参考面积(ha)",
            "observed_month_count": "观测月数",
        }
    )
    county_df = county_df[county_cols].sort_values(["地名"]).reset_index(drop=True)
    county_csv = workdir / "县级预测结果.csv"
    county_df.to_csv(county_csv, index=False, encoding="utf-8-sig")

    # 像元级动态参数表与折叠历史项表
    dyn_df = folded.dynamic_params.copy()
    dyn_csv = workdir / "动态参数表.csv"
    dyn_df.to_csv(dyn_csv, index=False, encoding="utf-8-sig")
    fold_df = folded.fold_table.copy()

    province = model["folded_formula_full_final"]
    if isinstance(province, float) and np.isfinite(province):
        message = (
            f"目标 {target_month}：全省平均预测单产 {province:.3f} t/亩"
            f"（{TRAIN_START_YEAR}-{TRAIN_END_YEAR} 榨季训练，{len(folded.dynamic_params)} 个像元动态特征）"
        )
    else:
        message = f"目标 {target_month}：运行完成（{TRAIN_START_YEAR}-{TRAIN_END_YEAR} 榨季训练）"

    return {
        "message": message,
        "metrics": metrics,
        "tables": [
            {"name": "县级预测结果", "columns": county_cols, "rows": _json_rows(county_df), "file": str(county_csv)},
            {"name": "像元级动态参数", "columns": list(dyn_df.columns), "rows": _json_rows(dyn_df), "file": str(dyn_csv)},
            {"name": "折叠进截距的县级历史项", "columns": list(fold_df.columns), "rows": _json_rows(fold_df)},
        ],
        "rasters": [
            {"name": "最终估产结果（t/亩）", "tif": str(final_tif)},
            {"name": "原始模型结果（t/ha）", "tif": str(raw_tif)},
        ],
    }
